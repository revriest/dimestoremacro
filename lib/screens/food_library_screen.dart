...
class _FoodLibraryScreenState extends State<FoodLibraryScreen> {
  ...
  Future<void> _toggleFavorite(Map<String, dynamic> food) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(isNowFavorite ? '⭐ Added to Quick Meals' : 'Removed from Quick Meals'),
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setDialogState) {
          final String grams = gramsCtrl.text.trim().isEmpty ? '100' : gramsCtrl.text.trim();
          final String macroLabel = useCustomGrams ? 'per ${grams}g' : 'per serving';
          final String nameHint = useCustomGrams ? 'e.g., Chicken Breast' : 'e.g., Protein Shake (1 serving)';
            ...
            children: [
              ...
              Padding(
                ...
                child: Align(
                  ...
                  child: Row(
                    children: [
                      if (isFav) const Icon(Icons.star_rounded, color: Colors.amber, size: 16),
                      if (isFav) const SizedBox(width: 6),
                      Text(food['name'] as String? ?? 'Meal', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)),
                    ],
                  ),
                ),
              ),
              Padding(
                ...
                child: Align(
                  alignment: Alignment.centerLeft,
                  child: Text('P ${p}g  •  C ${c}g  •  F ${f}g  •  $cal kcal', style: const TextStyle(color: Colors.white54, fontSize: 12)),
                ),
              ),
              const Divider(color: Colors.white12, height: 24),
              ListTile(
                leading: const Icon(Icons.add_circle_outline_rounded, color: Colors.blueAccent),
                title: const Text('Add to Today', style: TextStyle(color: Colors.white)),
                onTap: () async {
                  Navigator.pop(ctx);
              ...
              ListTile(
                leading: Icon(isFav ? Icons.star_rounded : Icons.star_outline_rounded, color: Colors.amber),
                title: Text(isFav ? 'Remove from Quick Meals' : 'Pin to Quick Meals', style: const TextStyle(color: Colors.white)),
                onTap: () {
                  Navigator.pop(ctx);
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.pop(ctx);
                  _confirmDeleteCustomMeal(food['id'] as int, food['name'] as String? ?? 'Meal');
                },
              ),
      ...
      body: _foods.isEmpty
          ? Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.restaurant_menu_rounded, size: 56, color: Colors.white.withValues(alpha: 0.15)),
                  const SizedBox(height: 16),
                  const Text('No custom meals yet', style: TextStyle(color: Colors.white54, fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 8),
                  const Text('Tap "New Meal" to add one', style: TextStyle(color: Colors.white30, fontSize: 13)),
                ],
              ),
            )
          : ListView.builder(
                  ...
                  confirmDismiss: (direction) async {
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (ctx) => AlertDialog(
                        backgroundColor: const Color(0xFF1C1C1E),
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                        title: const Text('Delete meal?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: Colors.white)),
                        content: Text('Remove "${food['name']}"?', style: const TextStyle(color: Colors.white70)),
                        actions: [
                          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel', style: TextStyle(color: Colors.grey))),
                          ElevatedButton(
                            style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
